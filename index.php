...
<?php if (is_front_page()) { ?>
	...
	if ($query->have_posts()):
		while ($query->have_posts()):
			$query->the_post(); ?>
			<section class="md:container mx-auto my-8 lg:my-24 w-full flex flex-col lg:flex-row md:rounded-md overflow-hidden">

				<div class="h-full md:w-6/12 items-center justify-center">
					<?php the_post_thumbnail('large', array('class' => 'w-full h-full object-cover hover:opacity-90 transition duration-300 ')); ?>
				</div>
